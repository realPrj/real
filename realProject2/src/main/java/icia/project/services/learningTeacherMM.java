			...
			if(dao.learningWANCheck(board) != 0) {
				...
				for(forI=0; forI < sizeDouble; forI++) {
					...
					for(forB = forB; forB < pageCount; forB++){//복사
						...
						sb.append("</td>");
						sb.append("<td style=\"text-align:center; color:blue\" type='button' name='cmClick' class=\"CTX\" id="+boardList.get(forB).getBoardCode()+" />" +"선생님 코멘트" +"</td>");
						/*sb.append("<td>");
						sb.append("<input style=\"text-align:center; color:blue\" type='button' name='cmClick' class=\"CTX\" id="+boardList.get(forB).getBoardCode()+" value='선생님 코멘트'  />");
						sb.append("</td>");*/
						sb.append("<td style=\"text-align:center\">");
						sb.append(check);
						sb.append("</td>");
	...
	private ModelAndView learningSTInformationPage(BoardBean board) { // 오답노트 학생별 정보 페이지
		...
		try {		
			...
			if(dao.allWANSum(board) != 0) {
				...
				board.setAllSum(dao.allWANSum(board));		// 학생이 물어본  문제 수
		...
		try {
			...
			for(int i=0; i<taglist.size(); i++) {
				if(taglist.size()!=0) {
					...
					sb.append("<td class=\"tag_delete\" onClick=\"tagDelete(\'"+bb.getBoardTitle()+"\',"+"\'"+bb.getBoardDate()+"\',"+"\'"+bb.getRoomCode()+"\',"+"\'"+bb.getBoardId()+"\',"+"\'"+taglist.get(i).getTagDate()+"\',"+"\'"+taglist.get(i).getTagId()+"\')\" />"+"삭제"+"</td>");
	...
	private ModelAndView learningTaskPage(BoardBean board) { // 과제 페이지
		...
		try {
			...
			if(board.getBoardCode() != null) {	// 게시글 내용,댓글 보여주기
				...
				sb.append("<td colspan=\"2\" class=\"title\" style=\"padding:20px 3px; border:none\">"+board.getBoardContent().replace("\r\n", "<br/>")+"</td>");
				sb.append("</tr>");
				sb.append("</table>");
				sb.append("</center>");
				...
				sb.append("<table id='tableText'>");
				sb.append("<tr>");
				sb.append("<td><button class='CTXbtn' onClick=\"update('"+board.getBoardCode()+"','"+board.getRoomCode()+"','"+board.getBoardTitle()+"','"+board.getBoardContent().replace("\r\n", "<br/>")+"')\">수정</button></td>");
				sb.append("<td>" + "<button class=\"CTXbtn_end\" onClick=deleteCXT("+board.getBoardCode()+","+board.getRoomCode()+") >" + "삭제" + "</button>" + "</td>");
				sb.append("</tr>");
				sb.append("</table>");
				sb.append("<br/>");
			...
			if(dao.learningTaskCheck(board) != 0) {    // 리스트 출력
				...
				for(forI=0; forI < sizeDouble; forI++) {
					...
					for(forB=forB; forB<pageCount; forB++) {
						...
						sb.append(forB+1);
						sb.append("</td>");
						sb.append("<td class=\"CTX\" id='click"+forB+"' onClick=\"questionCXT(\'"+ al.get(forB).getBoardCode() +"\')\">"+ al.get(forB).getBoardTitle() + "</td>");
						sb.append("<td>");
						sb.append(al.get(forB).getBoardDate());
						sb.append("</td>");
						sb.append("</tr>");    
